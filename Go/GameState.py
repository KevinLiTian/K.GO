...
class GameState:
    ...
    def __init__(self, size=19, komi=7.5, enforce_superko=False):
        ...
        self.hash_lookup = {
            WHITE: rng.randint(
                np.iinfo(np.uint64).max, size=(size, size), dtype="uint64"
            ),
            BLACK: rng.randint(
                np.iinfo(np.uint64).max, size=(size, size), dtype="uint64"
            ),
    ...
    def _update_hash(self, action, color):
        (x, y) = action
        self.current_hash = np.bitwise_xor(
            self.current_hash, self.hash_lookup[color][x][y]
        )
        ...
        if num_liberties_here == 0:
            ...
            for (nx, ny) in self._neighbors(action):
                ...
                group_has_other_liberties = (
                    len(self.liberty_sets[nx][ny] - set([action])) > 0
                )
    ...
    def is_ladder_capture(self, action, prey=None, remaining_attempts=80):
        ...
        if prey is None:
            ...
            neighbor_groups_stones = [
                next(iter(group)) for group in self.get_groups_around(action)
            ]
            potential_prey = [
                (nx, ny)
                for (nx, ny) in neighbor_groups_stones
                if (
                    self.board[nx][ny] == prey_player
                    and self.liberty_counts[nx][ny] == 2
                )
            ]
        else:
        ...
        for (prey_x, prey_y) in potential_prey:
            ...
            for prey_stone in tmp.group_sets[prey_x][prey_y]:
                for (nx, ny) in tmp._neighbors(prey_stone):
                    if (tmp.board[nx][ny] == hunter_player) and (
                        tmp.liberty_counts[nx][ny] == 1
                    ):
                        possible_escapes |= tmp.liberty_sets[nx][ny]

            if not any(
                tmp.is_ladder_escape(
                    (escape_x, escape_y),
                    prey=(prey_x, prey_y),
    ...
    def is_ladder_escape(self, action, prey=None, remaining_attempts=80):
        ...
        if prey is None:
            ...
            potential_prey = [
                ...
                if (
                    self.board[nx][ny] == prey_player
                    and self.liberty_counts[nx][ny] == 1
                )
            ]
        else:
            ...
            if action is not PASS_MOVE:
                ...
                for (nx, ny) in self._neighbors(action):
                    if (
                        ...
                        and len(self.liberty_sets[nx][ny]) == 0
                    ):
                        # capture occurred!
